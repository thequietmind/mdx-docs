...
`toolbar: false`. Style and position it through `sx`; any other props pass
through to the underlying MUI `IconButton`.
...
| _...rest_ | `IconButtonProps` | No | Forwarded to the underlying MUI `IconButton` (e.g. `color`, `size`) |

To build a custom control instead, the `useColorMode` hook exposes the same
...
`useColorMode` returns `{ darkMode, setDarkMode, toggleColorMode }`. Both helpers
work inside the default shell rendered by `createApp`. In a custom layout built
from `DocsProvider` and `MDXContent`, you supply your own theme state, so manage
...
standalone `ColorModeToggle` component (documented above) and position it
wherever you like. `footer: false` overrides the page only; the site-wide footer
